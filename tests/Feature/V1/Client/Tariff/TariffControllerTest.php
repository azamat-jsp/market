...
use App\Tbuy\Tariff\Enums\Permission;
...
class TariffControllerTest extends TestCase
{
    public function test_successfully_get_list(): void
    {
        ...
        $user->givePermissionTo([Permission::VIEW_TARIFF_LIST->value]);
    ...
    public function test_successfully_buy_tariff()
    {
         ...
         */
        $user = User::query()
            ->whereHas('company', fn(Builder $builder) => $builder->whereNull('deleted_at'))
            ->first();

        $user->givePermissionTo([
            Permission::VIEW_TARIFF_LIST->value,
            Permission::BUY_TARIFF->value
        ]);
